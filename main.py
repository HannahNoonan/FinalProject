...
highest_cals = calories_per_item.sort_values(["Calories", "Item", "Category"],
                                             ascending=[False, True, True]).drop_duplicates("Category")
print(highest_cals)
...
ax[1].scatter(McDonalds_menu['Category'], McDonalds_menu['Sodium (% Daily Value)'], color='g')
ax[0].tick_params(axis='x', rotation=90)
ax[1].tick_params(axis='x', rotation=90)
ax[0].set_xlabel('Category')
ax[1].set_xlabel('Category')
ax[0].set_title('Sodium')
ax[1].set_title('Sodium (% Daily Value)')
plt.show()
